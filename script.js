...
class Player{
    constructor(game){
        ...
        this.location={x:230,y:230, sizeX:80, sizeY:20}
    }
    draw(){
...
class BadCoin {
    constructor (game, direction) {
        this.game = game;
        this.screen = game.screen;
        this.location={x: (Math.floor(Math.random()*155)+155),y:(0)}
...
class BadCoinB {
    constructor (game) {
        this.game = game;
        this.screen = game.screen;
        this.location={x: (Math.floor(Math.random()*155)+155),y:(500)}
    }
    draw(){
        this.screen.fillStyle="#FF0000";
        var sizeX = 20;
...
class Keyboarder {
  ...
  Keyboarder.KEYS = { LEFT: 37, RIGHT: 39, UP: 38, DOWN: 40, SPACE: 32 };
